...
7. 「geo レプリケーション」ブレードで、「**確認して作成**」をクリックしてから「**作成**」をクリックします。 これで、セカンダリ サーバーとデータベースが作成されます。 ステータスを確認するには、ポータル上部のベル アイコンの下を確認します。 成功すると、状態が [デプロイが進行中です] から [デプロイが成功しました] に変化します。
